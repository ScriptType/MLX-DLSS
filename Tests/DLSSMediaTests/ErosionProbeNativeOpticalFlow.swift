...
public actor ErosionProbeNativeOpticalFlow {
  ...
  // EROSION-PROBE-BEGIN
  private var latestSnapshot: ErosionProbeSnapshot?

  func takeSnapshot() -> ErosionProbeSnapshot? {
  ...
  public init(width: Int, height: Int, mode: MediaMotion = .automatic) throws {
    ...
    if selected == .videoToolbox {
      ...
      do {
        session = try ErosionProbeVideoToolboxFlowSession(width: workingWidth, height: workingHeight)
  ...
  public func prepare(_ frame: MLXVideoFrame, index: Int, sceneCutThreshold: Float) async throws -> MLXVideoMotion? {
    ...
    if #available(macOS 15.4, *), let session = session as? ErosionProbeVideoToolboxFlowSession {
    ...
    // EROSION-PROBE-BEGIN
    let frozen = try FrozenErosionProbeVideoMotion(current: frame, previous: old.rgb,
      backward: backward, forward: forward, units: units == .flowPixels ? .flowPixels : .sourcePixels,
      sceneCutThreshold: sceneCutThreshold)
    latestSnapshot = try ErosionProbeSnapshot(production: motion, frozen: frozen,
      forward: forward, backward: backward, sourceIndex: index, previousIndex: old.index,
      randomAccessSubmitted: randomAccess || old.index + 1 != index)
    // EROSION-PROBE-END
...
private final class ErosionProbeVideoToolboxFlowSession {
