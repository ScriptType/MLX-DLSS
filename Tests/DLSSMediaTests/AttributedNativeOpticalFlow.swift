...
public actor AttributedNativeOpticalFlow {
  ...
  // FLOW-ATTRIBUTION-BEGIN
  private var latestObservation: NativeFlowStageObservation?
  func observation() -> NativeFlowStageObservation? {
    guard var result = latestObservation else { return nil }
    result.workingPixels = previous?.pixels
    return result
  }
  // FLOW-ATTRIBUTION-END
  public init(width: Int, height: Int, mode: MediaMotion = .automatic) throws {
    ...
    if selected == .videoToolbox {
      ...
      do {
        session = try AttributedVideoToolboxFlowSession(width: workingWidth, height: workingHeight)
  ...
  public func prepare(_ frame: MLXVideoFrame, index: Int, sceneCutThreshold: Float) async throws -> MLXVideoMotion? {
    // FLOW-ATTRIBUTION-BEGIN
    var trace = NativeFlowStageObservation(entryTicks: DispatchTime.now().uptimeNanoseconds)
    defer {
      trace.exitTicks = DispatchTime.now().uptimeNanoseconds
      latestObservation = trace
    }
    // FLOW-ATTRIBUTION-END
    ...
    // FLOW-ATTRIBUTION-BEGIN
    trace.packStartTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
    let fullSize = try await halfWriter.write(frame)
    // FLOW-ATTRIBUTION-BEGIN
    trace.packEndTicks = DispatchTime.now().uptimeNanoseconds
    trace.packCommandGPUSeconds = fullSize.gpuDurationSeconds
    trace.resizeStartTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
    let pixels = mode == .videoToolbox
      ? try await imageIO.resize(fullSize, width: workingWidth, height: workingHeight) : fullSize
    // FLOW-ATTRIBUTION-BEGIN
    trace.resizeEndTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
    let old = previous
    previous = (frame, pixels, index)
    // FLOW-ATTRIBUTION-BEGIN
    trace.hasPrevious = old != nil
    trace.completed = old == nil
    // FLOW-ATTRIBUTION-END
    guard let old else { return nil }
    let backward: MLXPixelBuffer, forward: MLXPixelBuffer
    let units: MLXVideoMotion.Units
    // FLOW-ATTRIBUTION-BEGIN
    trace.randomAccessSubmitted = randomAccess || old.index + 1 != index
    trace.vtStartTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
    if #available(macOS 15.4, *), let session = session as? AttributedVideoToolboxFlowSession {
    ...
    // FLOW-ATTRIBUTION-BEGIN
    trace.vtEndTicks = DispatchTime.now().uptimeNanoseconds
    trace.assessmentStartTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
    ...
    // FLOW-ATTRIBUTION-BEGIN
    trace.assessmentEndTicks = DispatchTime.now().uptimeNanoseconds
    // FLOW-ATTRIBUTION-END
...
private final class AttributedVideoToolboxFlowSession {
...
// FLOW-ATTRIBUTION-BEGIN
struct NativeFlowStageObservation: Sendable {
  let entryTicks: UInt64
  var exitTicks: UInt64 = 0
  var packStartTicks: UInt64 = 0
  var packEndTicks: UInt64 = 0
  var resizeStartTicks: UInt64 = 0
  var resizeEndTicks: UInt64 = 0
  var vtStartTicks: UInt64 = 0
  var vtEndTicks: UInt64 = 0
  var assessmentStartTicks: UInt64 = 0
  var assessmentEndTicks: UInt64 = 0
  var packCommandGPUSeconds: Double?
  var workingPixels: MLXPixelBuffer?
  var hasPrevious = false
  var randomAccessSubmitted: Bool?
  var completed = false
}
// FLOW-ATTRIBUTION-END
