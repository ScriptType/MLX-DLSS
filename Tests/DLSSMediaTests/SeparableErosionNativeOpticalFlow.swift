...
public actor SeparableErosionNativeOpticalFlow {
  ...
  public init(width: Int, height: Int, mode: MediaMotion = .automatic) throws {
    ...
    if selected == .videoToolbox {
      ...
      do {
        session = try SeparableErosionVideoToolboxFlowSession(width: workingWidth, height: workingHeight)
  ...
  public func prepare(_ frame: MLXVideoFrame, index: Int, sceneCutThreshold: Float) async throws -> SeparableErosionVideoMotion? {
    ...
    let units: SeparableErosionVideoMotion.Units
    if #available(macOS 15.4, *), let session = session as? SeparableErosionVideoToolboxFlowSession {
    ...
    let motion = try SeparableErosionVideoMotion(current: frame, previous: old.rgb, backward: backward,
...
private final class SeparableErosionVideoToolboxFlowSession {
