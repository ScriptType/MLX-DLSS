...
public final class FrozenErosionProbeVideoMotion: @unchecked Sendable {
  ...
  // EROSION-PROBE-BEGIN
  private(set) var preErodeConfidence: MLXArray?

  static func originalErosion(_ input: MLXArray, params: MLXArray) -> MLXArray {
    let count = input.dim(1) * input.dim(2)
    return Self.erode([input, params], grid: (count, 1, 1), threadGroup: (256, 1, 1),
  ...
  private static let importFlow = MLXFast.metalKernel(
    name: "mlxdlss_erosion_probe_flow_import", inputNames: ["flow", "params"], outputNames: ["vectors"],
    source: #"""
  ...
  private static let assess = MLXFast.metalKernel(
    name: "mlxdlss_erosion_probe_motion_quality", inputNames: ["current", "previous", "backward", "forward", "params"],
  ...
  private static let erode = MLXFast.metalKernel(
    name: "mlxdlss_erosion_probe_motion_erode", inputNames: ["input", "params"], outputNames: ["confidence"],
    source: #"""
  ...
  private static let resizeKernel = MLXFast.metalKernel(
    name: "mlxdlss_erosion_probe_guide_resize", inputNames: ["input", "params"], outputNames: ["output"],
    source: #"""
